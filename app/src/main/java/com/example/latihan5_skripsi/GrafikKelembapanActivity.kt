...
import com.example.latihan5_skripsi.databinding.ActivityGrafikKelembapanBinding
...
class GrafikKelembapanActivity : AppCompatActivity() {

    private lateinit var binding: ActivityGrafikKelembapanBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityGrafikKelembapanBinding.inflate(layoutInflater)
        ...
        updateTimeRunnable = object : Runnable {
            override fun run() {
                val format = SimpleDateFormat("HH:mm - EEEE, dd MMMM yyyy",
                    Locale("id", "ID"))
        ...
        // Ambil tanggal terbaru untuk filter default
    ...
    private fun tampilkanGrafikPerjam(tanggal: String) {
        DataCenter.getRiwayatByTanggal(tanggal) { list ->
            ...
            val sortedList = list.sortedByDescending { it.waktu }

            // list sudah diurutkan di DataCenter berdasarkan jam
            val entries = ArrayList<Entry>()
            val xLabels = ArrayList<String>()

            for ((i, item) in sortedList.withIndex()) {
                val y = item.kelembapan.toFloatOrNull() ?: 0f
            ...
            for (tanggal in filtered) {
                DataCenter.getRiwayatByTanggal(tanggal) { jamList ->
                    ...
                    val avg = if (jamList.isNotEmpty()) {
                        jamList.map {it.kelembapan.toFloatOrNull() ?: 0f
                        }.average().toFloat()
                    } else 0f
    ...
    private fun buatGrafik(
        ...
        val dataSet = LineDataSet(entries, "Kelembapan Udara").apply {
